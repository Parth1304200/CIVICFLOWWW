...
export function AdminLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Mapping ADMIN to internal admin email for Firebase compatibility
      const internalEmail = 'admin@civicsense.com';
      try {
        await login(internalEmail, password);
      } catch (err) {
        if (err.code === 'auth/user-not-found' || err.code === 'auth/invalid-credential') {
          await signup('System Admin', internalEmail, password);
        } else {
          throw err;
        }
      }
